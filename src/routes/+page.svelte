<script>
  ...
  const filters = {
    ...
    'vehicle': 'Vehicle', 'phone': 'Phone', 'email': 'Email', 'object': 'Object', 'url': 'URL', 'intel': 'Intel'
  };
  ...
  function stopResize() { isResizingSidebar = false;
...
<div class="flex flex-1 min-h-0 overflow-hidden">
    ...
    <div class="bg-gray-950 flex flex-col h-full overflow-hidden transition-all duration-300 flex-shrink-0" style="width: {isMapPanelCollapsed ? '40px' : mapPanelWidth + 'px'}">
      ...
      {:else}
        <div class="flex bg-gray-900/90 backdrop-blur-sm border-b border-cyan-600/30 flex-shrink-0">
            <button class="flex-1 py-3 text-xs font-medium transition-all relative {activeMapTab === 'map' ? 'text-cyan-300 bg-gray-950' : 'text-gray-400 hover:text-cyan-400 hover:bg-gray-800/50'}" on:click="{() => activeMapTab = 'map'}" title="Switch to Map (Ctrl+4)">MAP</button>
            <button class="flex-1 py-3 text-xs font-medium transition-all relative {activeMapTab === 'timeline' ? 'text-cyan-300 bg-gray-950' : 'text-gray-400 hover:text-cyan-400 hover:bg-gray-800/50'}" on:click="{() => activeMapTab = 'timeline'}" title="Switch to Timeline (Ctrl+5)">TIMELINE</button>
            <button class="flex-1 py-3 text-xs font-medium transition-all relative {activeMapTab === 'intel' ? 'text-cyan-300 bg-gray-950' : 'text-gray-400 hover:text-cyan-400 hover:bg-gray-800/50'}" on:click="{() => activeMapTab = 'intel'}" title="Switch to Intel (Ctrl+6)">INTEL</button>
            <button class="flex-1 py-3 text-xs font-medium transition-all relative {activeMapTab === 'network' ? 'text-cyan-300 bg-gray-950' : 'text-gray-400 hover:text-cyan-400 hover:bg-gray-800/50'}" on:click="{() => activeMapTab = 'network'}" title="Switch to Network (Ctrl+2)">NETWORK</button>
  ...
  <div class="bg-gray-950 flex flex-col h-full overflow-hidden flex-shrink-0" style="width: {sidebarWidth}px">
    <div class="flex bg-gray-900/90 backdrop-blur-sm border-b border-cyan-600/30 flex-shrink-0">
      <button class="flex-1 py-3 text-xs font-medium transition-all relative {activeTab === 'entities' ? 'text-cyan-300 bg-gray-950' : 'text-gray-400 hover:text-cyan-400 hover:bg-gray-800/50'}" on:click="{() => activeTab = 'entities'}" title="Switch to Entities (Ctrl+1)">ENTITIES</button>
      <button class="flex-1 py-3 text-xs font-medium transition-all relative {activeTab === 'briefcase' ? 'text-cyan-300 bg-gray-950' : 'text-gray-400 hover:text-cyan-400 hover:bg-gray-800/50'}" on:click="{() => activeTab = 'briefcase'}">BRIEFCASE</button>
      <button class="flex-1 py-3 text-xs font-medium transition-all relative {activeTab === 'analysis' ? 'text-cyan-300 bg-gray-950' : 'text-gray-400 hover:text-cyan-400 hover:bg-gray-800/50'}" on:click="{() => activeTab = 'analysis'}" title="Switch to Analysis (Ctrl+3)">ANALYSIS</button>
    </div>
