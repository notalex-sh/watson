    ...
    function save() {
        ...
        if (editingEntity) {
            if (editingEntity.itemType === 'event') {
                ...
                links.update(l => {
                    const otherLinks = l.filter(link => link.from !== editingEntity.id && link.to !== editingEntity.id);
                    const newLinks = allLinkedIds.map(linkedId => ({ from: editingEntity.id, to: linkedId }));
