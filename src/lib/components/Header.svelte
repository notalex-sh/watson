<script>
  import { notes, entities, links, entityIdCounter } from '$lib/stores';
  ...
  function saveProject() {
    exportProject($notes, $entities, $links);
  ...
  function openProject(event) {
    ...
    if (file) {
      importProject(file, (data) => {
        ...
        // Update entity ID counter to avoid conflicts
...
<header class="h-[57px] bg-gray-900 border-b border-cyan-600/30 px-6 py-3 flex justify-between items-center relative z-10">
  ...
  <div class="flex items-center gap-4">
    ...
    <button 
      class="w-8 h-8 rounded bg-gray-800/80 backdrop-blur-sm hover:bg-cyan-600/20 border border-cyan-500/50 hover:border-cyan-400 flex items-center justify-center text-cyan-400 hover:text-cyan-300 transition-all duration-200 hover:shadow-lg hover:shadow-cyan-500/30"
    ...
    <div class="bg-gray-900/95 backdrop-blur-md border border-cyan-400 rounded-lg p-6 max-w-md mx-4 shadow-2xl shadow-cyan-500/30 max-h-[90vh] overflow-y-auto" on:click|stopPropagation>
      ...
      <div class="space-y-3 text-sm text-gray-300">
        ...
        <p class="text-gray-400 text-xs">
            ...
            <a href="https://github.com/notalex-sh/watson" target="_blank" class="text-cyan-300 underline hover:text-cyan-200">
        ...
        <div class="mt-4 pt-4 border-t border-cyan-600/30">
          ...
          <ul class="text-xs text-gray-400 space-y-1">
            ...
            <li><span class="text-cyan-300 font-mono">/</span> - Quick insert entity (in notes)</li>
            <li><span class="text-cyan-300 font-mono">Click entity</span> - View details</li>
            <li><span class="text-cyan-300 font-mono">Scroll</span> - Zoom network view</li>
          </ul>
        </div>
        <p class="text-gray-400 text-xs pt-2">
            Watson was created by 
            <a href="https://www.notalex.sh" target="_blank" class="text-cyan-300 underline hover:text-cyan-200">
                notalex.sh</a>.
        </p>
      </div>
      <button 
