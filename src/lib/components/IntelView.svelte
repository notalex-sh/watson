...
<div class="flex flex-col h-full bg-gray-900/30">
    {#if !presentationMode}
        <div class="p-4 text-center border-b border-cyan-600/30">
            <p class="text-xs text-cyan-500/70">Press <span class="font-bold text-cyan-400">Shift + Tab</span> to add new Intel</p>
            <p class="text-xs text-gray-500 mt-1">To edit intel items, please use the main entity list.</p>
        </div>
    {/if}
 ...
 backdrop-blur-sm border rounded-lg p-3 border-green-500/50 hover:border-green-400 transition-all duration-300">
                        <div>
             ...
             <div class="flex flex-wrap gap-2 mb-3">
                                    {#each getLinkedItems(intel.id) as entity}
                                        <span class="bg-purple-500/30 text-purple-300 px-2 
 py-1 rounded text-xs">{entity.content}</span>
                                    {/each}
                                </div>
                            </div>
